...
public:
  ...
  StvorkaElement(uint8_t* array) {
    setPins(new uint8_t[3]{ array[0], array[1], array[2] });
  ...
  StvorkaElement(uint8_t openPin, uint8_t closePin, uint8_t valuePin) {
    setPins(new uint8_t[3]{ openPin, closePin, valuePin });
    setPinModes();
    setBoundaries(255, 0);
    setConsts(150, 4);
    getValue();
  ...
  */
  uint8_t findBoundaries(uint8_t interruptionTime) {
    ...
    if (Serial.read() != 'y') {
      ...
      setPins(new uint8_t[3]{ PINS[1], PINS[0], PINS[2] });
      closeSrvorka();
    }
