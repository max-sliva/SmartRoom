...
#include "LockHandler.h"
  ...
  void LockHandler::setLockedState(boolean value) {
    if (locked != value) {
      if (value == true) {
        // because of goal to lock door
        revokeAccess();
      }
      else {
        // without grantAccessAction just to unlock door
        locked = false;
        stateWritePass = false;
        setButtonValue(LOW);
        resetPassword();
  ...
  LockHandler::LockHandler(uint8_t buttonPinIn, uint8_t buttonPinOut, uint8_t* _ledPins, Keypad* keypad,
              char* password, uint8_t length, char _specialSymbol) : DeviceObject(544) {
    this_keypad = keypad;
